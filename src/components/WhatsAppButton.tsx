...
export default function WhatsAppButton() {
  ...
  return (
    <motion.a href="https://wa.me/972546302880?text=Hi%2C%20I%27m%20interested%20in%20GATE%20Jerusalem" target="_blank" rel="noopener noreferrer" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2, type: "spring" }} className={`fixed left-6 z-50 group flex items-center gap-0 hover:gap-3 transition-all duration-300 ${stickyBarVisible ? "bottom-[72px]" : "bottom-6"} md:bottom-6`} title={dict.whatsapp.label}>
      <span className="overflow-hidden whitespace-nowrap bg-white text-navy text-sm font-medium px-3 py-2 rounded-full shadow-lg max-w-0 group-hover:max-w-40 group-hover:px-3 opacity-0 group-hover:opacity-100 transition-all duration-300 hidden md:block">{dict.whatsapp.label}</span>
      <div className="bg-[#25D366] hover:bg-[#20bd5a] text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow">
